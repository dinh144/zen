...
const tile = p.locator("figure", { hasText: /bánh mì|e2e/i }).first()
...
await p.locator("figure", { hasText: /bánh mì|e2e/i }).first().locator("button.sheet").click()
await p.getByRole("dialog").getByRole("button", { name: `+ ${spaceName}` }).click()
await p.getByRole("dialog").getByRole("button", { name: `− ${spaceName}` }).waitFor({ timeout: 5000 })
