// Builds the OpenAPI document from the route schemas in lib/schemas.ts. Pure (no I/O) so both
// scripts/openapi.ts (writes apps/web/openapi.json) and lib/openapi.test.ts (completeness check)
// can import it.
...
export const buildDocument = (): ReturnType<typeof createDocument> =>
  createDocument({
    openapi: "3.1.0",
    info: {
      title: "zen API",
    ...
    servers: [{ url: "/api" }],
    components: {
      // Referenced only from McpRequest's description (MCP dispatches by method name, not by path),
      // so listed explicitly here to appear as components the tool arguments actually are.
      schemas: { McpSearchArgs, McpGetCardArgs, McpSaveCardArgs, McpAskArgs },
      securitySchemes: {
        cookieAuth: {
          type: "apiKey",
          in: "cookie",
    ...
    paths: {
      ...
      "/cards": {
        ...
        post: {
          ...
          responses: { "201": ok("Created", CardResponse), "400": err("Invalid card"), "429": err("Daily cap reached") },
        },
      },
      "/cards/{id}": {
        get: {
      ...
      "/ask": {
        post: {
          ...
          responses: { "200": ok("An answer, a grilled question, a running task's result, or none.", AskReply), "400": err("No question") },
        },
      },
      "/ask/answer": {
        post: {
      ...
      "/log/{id}": {
        delete: {
          ...
          security: auth,
          requestParams: { path: idParam },
          responses: { "200": ok("OK", Ok), "404": err("Nothing to undo") },
        },
      },
      "/settings": {
        patch: {
      ...
      "/file/{name}": {
        get: {
          ...
          security: [],
          description:
            "No auth: the file name is an unguessable capability (a random id), the same model a signed URL uses. " +
            "Anyone holding the name can read the file, and no one can guess a name they were not given.",
      ...
      "/mcp": {
        post: {
          summary: "Model Context Protocol server (JSON-RPC 2.0) — zen as memory for other AI assistants",
          security: auth,
          requestBody: { content: { "application/json": { schema: McpRequest } } },
          responses: { "200": ok("A JSON-RPC 2.0 result or error.", McpResponse) },
        },
      },
    },
  })
