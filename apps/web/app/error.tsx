...
export default function Error({ error, retry }: { error: Error & { digest?: string }; retry: () => void }) {
  ...
  return (
    <div role="alert" className="mx-auto flex min-h-screen max-w-sm flex-col justify-center gap-6 px-6">
      ...
      <p className="font-display text-[19px] leading-relaxed">{t("errors", "broke")}</p>
      <button onClick={retry} className="text-primary w-fit text-[12px] tracking-[0.25em] lowercase">
        {t("errors", "retry")}
      </button>
    </div>
  )
