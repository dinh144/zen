...
async function guard(res) {
  if (res.status === 401) await forgetSession()
  return res
}
...
async function save(body) {
  ...
  try {
    ...
    // zen is unreachable — falls through to the failure notification below
  }
  chrome.notifications.create({
    type: "basic",
    iconUrl: "icon.png",
    title: ok ? "Saved to zen" : "zen could not save that",
...
async function saveImage(src, pageUrl) {
  ...
  try {
    ...
    ok = (await guard(await fetch(`${base}/api/upload`, { method: "POST", body: form, credentials: "include" }))).ok
  } catch {
    // zen is unreachable, or the image fetch itself failed — falls through below
  }
  chrome.notifications.create({
    type: "basic",
    iconUrl: "icon.png",
    title: ok ? "Image saved to zen" : "zen could not save that image",
    message: pageUrl,
  })
}
